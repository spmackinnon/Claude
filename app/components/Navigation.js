...
export default function Navigation({ current, onNavigate, onMapOpen, onFaqOpen }) {
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex flex-col w-56 min-h-screen bg-white border-r border-stone-200 fixed left-0 top-0 z-30">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <span className={active ? 'text-sage-600' : 'text-stone-400'}>
                  {item.icon(active)}
                </span>
                <div>
                  <div className={`text-sm font-medium ${active ? 'text-sage-700' : ''}`}>
                    {item.label}
                  </div>
                  <div className={`text-[11px] ${active ? 'text-sage-500' : 'text-stone-400'}`}>
                    {item.sublabel}
                  </div>
                </div>
              </button>
        ...
        <div className="px-3 pb-5 space-y-1 border-t border-stone-100 pt-3">
          <button
            onClick={onMapOpen}
            className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-stone-400 hover:bg-stone-50 hover:text-stone-600 transition-all text-sm"
          >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 z-30 flex">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              {item.icon(active)}
              <span className={`text-[10px] font-medium ${active ? 'text-sage-600' : 'text-stone-400'}`}>
                {item.label}
              </span>
            </button>
          );
