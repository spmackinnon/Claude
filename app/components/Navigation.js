...
export default function Navigation({ current, onNavigate, onMapOpen, onFaqOpen, onLibraryOpen, onProjectLibraryOpen }) {
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex flex-col w-56 min-h-screen bg-white border-r border-stone-200 fixed left-0 top-0 z-30">
        ...
        <div className="px-3 pb-5 space-y-1 border-t border-stone-100 pt-3">
          ...
          <button onClick={onProjectLibraryOpen} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all text-sm ${current === 'project-library' ? 'bg-sage-50 text-sage-700' : 'text-stone-400 hover:bg-stone-50 hover:text-stone-600'}`}>
